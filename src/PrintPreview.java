...
class PrintPreview extends JFrame implements ActionListener, ChangeListener, ItemListener
{
	...
	private void createPreview()
	{
		...
		{
			jcb.addItem(""+(i+1));
			page[i] = new Page(i, size);JScrollPane sc= new JScrollPane();sc.setViewportView(page[i]);
			p.add(""+(i+1),sc);
		}
		setTopPanel();
		...
		int pNo=jcb.getSelectedIndex();
		if(pNo<0||pNo>=page.length)
		{
			JOptionPane.showMessageDialog(this,"Sorry !!! The Xtreme Pad is Blank.\nPlease Don't Waste Paper !!!!",
				"Warning !!!",2);
		}
		else if(pNo>=0&&pNo<page.length){this.setVisible(true);page[pNo].refreshScale();}
	}
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		else if(o == printThisPage){printCurrentPage();}
		else if(o == back)
		{
			jcb.setSelectedIndex(jcb.getSelectedIndex() == 0 ? 0:jcb.getSelectedIndex()-1);
			if(jcb.getSelectedIndex() == 0){back.setEnabled(false);}
		}
		else if(o == forward)
		{
	...
	public void printCurrentPage() 
	{
		try 
		{
			...
			if(pj.printDialog(pra)){pj.print(pra);}
		}
		catch(Exception ex)
		{
			JOptionPane.showMessageDialog(null,"An Error Occured During Printing. \nPlease Check The Printer Connection and Try Again. "
